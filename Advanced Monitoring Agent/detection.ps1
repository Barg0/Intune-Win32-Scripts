...
# Initialize a flag to indicate if the program is found
$programFound = $false
...
foreach ($registryPath in $registryPaths) {
    ...
    foreach ($subkey in $subkeys) {
        # Get the DisplayName and DisplayVersion values
        ...
        # Check if the values match the specified program and are of type REG_SZ
        if ($displayName -eq $softwareName) {
            $displayNameType = (Get-ItemProperty -Path $subkey.PSPath -Name "DisplayName" -ErrorAction SilentlyContinue).PSObject.Properties["DisplayName"].TypeNameOfValue
            # $displayVersionType = (Get-ItemProperty -Path $subkey.PSPath -Name "DisplayVersion" -ErrorAction SilentlyContinue).PSObject.Properties["DisplayVersion"].TypeNameOfValue

            if ($displayNameType -eq "System.String") {
                $programFound = $true
                break
            }
        }
...
if ($programFound) {
    Write-Log "$softwareName is installed." "Success"
    Complete-Script -ExitCode 0
} else {
    Write-Log "$softwareName is not installed." "Error"
