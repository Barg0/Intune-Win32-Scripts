...
# ---------------------------[ Install ]-------------------------------------
Write-Log "Install: $applicationName" -Tag "Run"
...
try {
    ...
    if ($null -ne $process) {
        ...
        Write-Log "EXE exit $($process.ExitCode): $exitMeaning" -Tag "Info"

        if ($process.ExitCode -eq 0) {
            Write-Log "EXE OK." -Tag "Success"
        }
        elseif ($process.ExitCode -eq 3010 -or $process.ExitCode -eq 1641) {
            Write-Log "EXE OK; reboot $($process.ExitCode) (Intune soft-reboot)." -Tag "Info"
        }
        elseif ($process.ExitCode -eq 1602) {
            Write-Log "User canceled (1602)." -Tag "Info"
        }
        else {
            Write-Log "EXE exit $($process.ExitCode): $exitMeaning" -Tag "Info"
        }
    ...
    Write-Log "EXE done; verify reg." -Tag "Info"
}
catch {
